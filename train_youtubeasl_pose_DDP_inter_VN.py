...
class YoutubeASLPoseDataset(Dataset):
    def __init__(self, arg_dict, tokenizer, phase='train', split='train'):
        ...
        self.label_path = os.path.join(f'/home/grt/youtube-asl_data/data/tsv_files/new_youtube-asl_v1_1.tsv')
        ...
        self.feat_path = os.path.join(f'/home/grt/GloFE/OpenASL/ytasl/mmpose')
        print(f'split: {split}')
        ...
        print(f'number vid: {self.video_names}')
        self.vn_vocab = 304
        self.matched_VNs = json.load(open('notebooks/ytasl-v1.0/uncased_filtred_glove_VN_matched_train.json', 'r'))
        self.vn_to_idx = {}
        with open('notebooks/ytasl-v1.0/uncased_filtred_glove_VN_idxs.txt', 'r') as f:
            content = f.readlines()
            for line in content:
        ...
        print(len(self.translation_token_ids))
        all_l = torch.tensor([len(tk) for tk in self.translation_token_ids])
        print(all_l)
...
def eval(dataset, model, args, device, split, save_results: bool = False,
    ...
    #print(f"ref: {ref_text_tokens}")
    #print(f"gen: {gen_text_tokens}")
...
def main():
    ...
    parser.add_argument(
        '--work_dir_prefix', default='/home/grt/GloFE', help='path to work_dir')
    parser.add_argument('--work_dir', default='checkpoints',
                        help='<dataset>/<exp_name>')
    parser.add_argument(
        '--prefix', type=str, default='youtube_asl', help='prefix for saved filenames')
    ...
    parser.add_argument('--weights', type=str, default='/home/grt/GloFE/pre-trained/vn_model/glofe_vn_openasl.pt')
    ...
    # Datasets
    parser.add_argument('--feat_path', type=str, default='/home/grt/GloFE/OpenASL/mmpose',
    ...
    parser.add_argument('--tokenizer', type=str,
                        default='/home/grt/GloFE/notebooks/openasl-v1.0/openasl-bpe25000-tokenizer-uncased')
    ...
    if args.phase == 'train':
        # [Init datasets]
        dataset = YoutubeASLPoseDataset(vars(args), tokenizer=tokenizer, phase='train', split='train')
        dataset_dev = YoutubeASLPoseDataset(vars(args), tokenizer=tokenizer, phase='test', split='valid')
        dataset_test = YoutubeASLPoseDataset(vars(args), tokenizer=tokenizer, phase='test', split='test')

        # pass the namespace as a dict
        model = construct_model(model_cls, args, distributed)
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        
        print(f"NUMBER PARAMETERS: {sum(p.numel() for p in model.parameters())}")
        if weights:
        ...
        assert weights is not None, f'{args.weights} does not exist'
        ...
        dataset_valid = YoutubeASLPoseDataset(
            config, tokenizer=tokenizer, phase='test', split='valid')
        dataset_test = YoutubeASLPoseDataset(
