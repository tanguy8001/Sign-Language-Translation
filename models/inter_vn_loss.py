...
def batched_cosine_sim(a, b, dim=-1):
    ...
    b_norm = F.normalize(b, p=2, dim=dim)
    if b_norm.shape[1] == 2 and b_norm.shape[3] == 1:
        # Assume we want to collapse the second dimension (2) and the last singleton dimension
        b_norm = b_norm.squeeze(-1).mean(dim=1, keepdim=True)  # Taking mean if dimension 2 is meaningful
...
class InterSampleContrastiveLoss(nn.Module):
    ...
    def forward(self, visual_feat, visual_feat_len, vns, vn_len):
        ...
        if len(pos_idx.shape) == 1:
            # Handle the case when `pos_idx` is 1-dimensional
            pos_xatten_out = xatten_out[pos_idx]  # Assuming `pos_idx` directly indexes one dimension
            neg_xatten_out = xatten_out[neg_idx]
        else:
    ...
    def random_from_mask(self, a, num_per_row=1):
        ...
        if valid_idx.size(0) == 0:
            # If there are no valid indices, return an empty tensor appropriate for downstream processing
            return torch.tensor([], device=a.device, dtype=torch.long)
        ret = []
...
class GloVeEmbedding(nn.Embedding):
    def __init__(self, vocab, embed_dim, path, requires_grad=True):
        ...
        with open(path, 'rb') as f:
            glove_np = pkl.load(f)
            print("GloVe shape:", glove_np.shape)
            print("Expected vocab size:", vocab)
            print("Expected embedding dimension:", embed_dim)
